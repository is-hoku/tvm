...
#define gemmini_extended_mvout_spad(dst_addr, dst_stride, src_addr, cols,      \
                                    rows)                                      \
  ROCC_INSTRUCTION_RS1_RS2(                                                    \
      XCUSTOM_ACC, ((uint64_t)(dst_stride) << 32) | (uint64_t)(dst_addr),      \
      ((uint64_t)(rows) << (ADDR_LEN + 16)) | ((uint64_t)(cols) << ADDR_LEN) | \
          (uint64_t)(src_addr),                                                \
      k_MVOUT_SPAD)
...
#define gemmini_loop_ws_spad(                                                  \
    I, J, K, pad_I, pad_J, pad_K, A, B, D, C, A_transpose, B_transpose,        \
    full_C, low_D, ex_accumulate, act, a_spad_id, b_spad_id, is_resadd, skips) \
  {ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,                                       \
                            ((uint64_t)(pad_K) << 32) |                        \
                                ((uint64_t)(pad_J) << 16) | (uint64_t)(pad_I), \
                            ((uint64_t)(K) << 32) | ((uint64_t)(J) << 16) |    \
       ...
       ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, A, B, k_LOOP_WS_CONFIG_SPAD_AB)   \
           ROCC_INSTRUCTION_RS1_RS2(                                           \
               XCUSTOM_ACC,                                                    \
               ((uint64_t)(a_spad_id) << 18) | ((uint64_t)(b_spad_id) << 16) | \
                   ((uint64_t)(act) << 8) | ((low_D) << 2) | ((full_C) << 1) | \
                   (ex_accumulate),                                            \
               ((uint64_t)(C) << 32) | 0x200U | (skips) | ((is_resadd) << 2) | \
                   ((B_transpose) << 1) | (A_transpose),                       \
               k_LOOP_WS)}
...
_STATIC void tiled_conv_auto(int batch_size, int in_row_dim, int in_col_dim,
                             int in_channels, int out_channels, int out_row_dim,
                             int out_col_dim, int stride, int input_dilation,
                             int kernel_dilation, int padding, int kernel_dim,
                             bool wrot180, bool trans_output_1203,
...
_STATIC void tiled_conv_dw_auto(int batch_size, int in_row_dim, int in_col_dim,
                                int channels, int out_row_dim, int out_col_dim,
                                int stride, int padding, int kernel_dim,
...
_STATIC void tiled_norm_auto(const size_t I, const size_t J, const acc_t *in,
