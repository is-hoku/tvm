...
static elem_t conv1_weights[CONV1_FILTERS][INPUT_CHANNELS][CONV1_KERNEL]
                           [CONV1_KERNEL];
static acc_t conv1_bias[CONV1_FILTERS];
static elem_t conv2_weights[CONV2_FILTERS][CONV1_FILTERS][CONV2_KERNEL]
                           [CONV2_KERNEL];
static acc_t conv2_bias[CONV2_FILTERS];
static elem_t conv3_weights[CONV3_FILTERS][CONV2_FILTERS][CONV3_KERNEL]
                           [CONV3_KERNEL];
    ...
    for (int ch = 0; ch < INPUT_CHANNELS; ch++) {
      for (int kr = 0; kr < CONV1_KERNEL; kr++) {
        for (int kc = 0; kc < CONV1_KERNEL; kc++) {
          ...
          conv1_weights[f][ch][kr][kc] = (elem_t)((f + kr + kc + ch) % 21 - 10);
        }
      }
    }
    ...
    for (int ch = 0; ch < CONV1_FILTERS; ch++) {
      for (int kr = 0; kr < CONV2_KERNEL; kr++) {
        for (int kc = 0; kc < CONV2_KERNEL; kc++) {
          conv2_weights[f][ch][kr][kc] = (elem_t)((f + kr + kc + ch) % 21 - 10);
        }
      }
    }
    conv2_bias[f] = 1;
    ...
    for (int ch = 0; ch < CONV2_FILTERS; ch++) {
      for (int kr = 0; kr < CONV3_KERNEL; kr++) {
        for (int kc = 0; kc < CONV3_KERNEL; kc++) {
          conv3_weights[f][ch][kr][kc] = (elem_t)((f + kr + kc + ch) % 21 - 10);
        }
      }
    }
    conv3_bias[f] = 1;
    ...
    for (int inc = 0; inc < in_channels; inc++) {
      for (int krow = 0; krow < kernel_dim; krow++) {
        for (int kcol = 0; kcol < kernel_dim; kcol++) {
          ...
          weights_mat[wmatrow][outc] = weights[outc][inc][krow][kcol];
        }
      }
    }
  }
...
void cpu_nlb(elem_t *input, elem_t *output) {
  // Theta path: 1x11x11x64 -> conv -> 1x32x9x9
  static elem_t nlb_theta_out[CONV2_FILTERS][CONV1_DIM][CONV1_DIM];

  tiled_conv_auto(BATCH, CONV1_DIM, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS,
  ...
  // printf("Conv theta \n");
  printf("nlb_theta_out: %d, %d, %d", nlb_theta_out[0][0][0],
         nlb_theta_out[0][0][1], nlb_theta_out[0][0][2]);
  ...
  for (int c = 0; c < CONV2_FILTERS; c++) {
    for (int h = 0; h < CONV1_DIM; h++) {
      for (int w = 0; w < CONV1_DIM; w++) {
        ...
        nlb_theta_reshaped[c][idx] = nlb_theta_out[c][h][w];
      }
    }
  }
  ...
  static elem_t nlb_phi_out[CONV2_FILTERS][CONV1_DIM][CONV1_DIM];
  tiled_conv_auto(BATCH, CONV1_DIM, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS,
                  CONV1_DIM, CONV1_DIM, 1, 1, 1, 0, CONV1_KERNEL, false, false,
  ...
  // printf("Conv phi \n");
  printf("nlb_phi_out: %d, %d, %d", nlb_phi_out[0][0][0], nlb_phi_out[0][0][1],
         nlb_phi_out[0][0][2]);
  ...
  for (int c = 0; c < CONV2_FILTERS; c++) {
    for (int h = 0; h < CONV1_DIM; h++) {
      for (int w = 0; w < CONV1_DIM; w++) {
        ...
        nlb_phi_reshaped[c][idx] = nlb_phi_out[c][h][w];
      }
    }
  }
  ...
  static elem_t nlb_g_out[CONV2_FILTERS][CONV1_DIM][CONV1_DIM];
  tiled_conv_auto(BATCH, CONV1_DIM, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS,
                  CONV1_DIM, CONV1_DIM, 1, 1, 1, 0, CONV1_KERNEL, false, false,
  ...
  // printf("Conv g \n");
  printf("nlb_g_out: %d, %d, %d", nlb_g_out[0][0][0], nlb_g_out[0][0][1],
         nlb_g_out[0][0][2]);
  ...
  for (int c = 0; c < CONV2_FILTERS; c++) {
    for (int h = 0; h < CONV1_DIM; h++) {
      for (int w = 0; w < CONV1_DIM; w++) {
        ...
        nlb_g_reshaped[c][spatial_idx] = nlb_g_out[c][h][w];
      }
    }
  }
  ...
  // printf("Matmul g \n");
  printf("attended_output: %d, %d, %d", attended_output[0][0],
         attended_output[0][1], attended_output[0][2]);

  // Reshape back to spatial format: 81x32 -> 1x32x9x9
  static elem_t theta_phi_g_output[CONV2_FILTERS][CONV1_DIM][CONV1_DIM];
  for (int c = 0; c < CONV2_FILTERS; c++) {
    for (int idx = 0; idx < CONV1_2D; idx++) {
      int h = idx / CONV1_DIM;
      int w = idx % CONV1_DIM;
      theta_phi_g_output[c][h][w] = attended_output[c][idx];
    }
  }

  // printf("Reshape theta, phi, and g \n");
  printf("theta_phi_g_output: %d, %d, %d", theta_phi_g_output[0][0][0],
         theta_phi_g_output[0][0][1], theta_phi_g_output[0][0][2]);

  static elem_t nlb_output[CONV1_DIM][CONV1_DIM][64];
  tiled_conv_auto(BATCH, CONV1_DIM, CONV1_DIM, CONV2_FILTERS, CONV1_FILTERS,
                  CONV1_DIM, CONV1_DIM, 1, 1, 1, 0, 1, false, false, false,
                  false, false, theta_phi_g_output, (elem_t *)nlb_out_weights,
                  nlb_out_bias, (elem_t *)nlb_output, NO_ACTIVATION,
                  ACC_SCALE_IDENTITY, 0, 0, 0, CPU);

  // printf("Conv theta, phi, and g \n");
  printf("nlb_output: %d, %d, %d", nlb_output[0][0][0], nlb_output[0][0][1],
         nlb_output[0][0][2]);
...
void cpu_inference(elem_t *input, elem_t *output) {
  ...
  static elem_t conv1_out[CONV1_DIM][CONV1_DIM]
                         [CONV1_FILTERS]; // 11x11 -> 9x9x64 (3x3, no pad)
  static elem_t nlb_out[CONV1_FILTERS][CONV1_DIM]
                       [CONV1_DIM]; // NLB output: 64x9x9
  ...
  tiled_conv_auto(BATCH, INPUT_DIM, INPUT_DIM, INPUT_CHANNELS, CONV1_FILTERS,
                  CONV1_DIM, CONV1_DIM, 1, 1, 1, 0, CONV1_KERNEL, false, false,
                  false, false, false, input, (elem_t *)conv1_weights,
                  conv1_bias, (elem_t *)conv1_out, NO_ACTIVATION,
                  ACC_SCALE_IDENTITY, 0, 0, 0, CPU);

  printf("conv1_out: %d, %d, %d", conv1_out[0][0][0][0], conv1_out[0][0][0][1],
         conv1_out[0][0][0][2]);
  ...
  printf("conv2_out: %d, %d, %d", conv2_out[0][0][0][0], conv2_out[0][0][0][1],
         conv2_out[0][0][0][2]);
  // Conv3: 7x7x32 -> 5x5x8 (3x3 kernel, no padding) + LeakyReLU
  // printf("  Layer 3: Conv2D (3x3, 8 filters) + LeakyReLU -> 5x5x8\n");
  tiled_conv_auto(BATCH, CONV2_DIM, CONV2_DIM, CONV2_FILTERS, CONV3_FILTERS,
  ...
  printf("conv3_out: %d, %d, %d", conv3_out[0][0][0][0], conv3_out[0][0][0][1],
         conv3_out[0][0][0][2]);
  // Flatten: 5x5x8 -> 200
  ...
  printf("fc1_out: %d, %d, %d", fc1_out[0], fc1_out[1], fc1_out[2]);
  ...
  printf("fc2_out: %d, %d, %d", fc2_out[0], fc2_out[1], fc2_out[2]);
  ...
  printf("fc3_out: %d, %d, %d", fc3_out[0], fc3_out[1], fc3_out[2]);
  ...
  printf("fc4_out: %d, %d, %d", fc4_out[0], fc4_out[1], fc4_out[2]);
...
void gemmini_nlb(elem_t *input, elem_t *output) {
  ...
  // printf("Conv theta \n");
  printf("nlb_theta_out: %d, %d, %d", nlb_theta_out[0][0][0][0],
  ...
  // printf("Conv phi \n");
  printf("nlb_phi_out: %d, %d, %d", nlb_phi_out[0][0][0][0],
  ...
  // G path: 1x11x11x64 -> conv -> 1x9x9x32 -> reshape to 1x32x81
  static elem_t nlb_g_out[CONV1_DIM][CONV1_DIM][CONV2_FILTERS];
  gemmini_conv2d(BATCH, CONV1_DIM, CONV1_DIM, CONV1_FILTERS, CONV2_FILTERS,
...
void gemmini_inference(elem_t *input, elem_t *output) {
  ...
  printf("output: %d, %d, %d", output[0], output[1], output[2]);
  // printf("Gemmini Inference: Complete\n");
}
