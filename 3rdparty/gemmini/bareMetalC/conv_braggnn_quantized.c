...
void init_random_acc(elem_t *buf, int len) {
  elem_t i = 0;
  for (elem_t *ptr = buf; ptr < buf + len; ptr++) {
    // *ptr = (rand() % 32) - 16;
#ifdef FAST
...
void init_zeros_acc(elem_t *buf, int len) {
  for (elem_t *ptr = buf; ptr < buf + len; ptr++) {
    *ptr = 0;
  }
}
...
int main() {
  ...
  static elem_t bias[OUT_CHANNELS];
  ...
  if (NO_BIAS)
    init_zeros_acc(&bias[0], sizeof(bias) / sizeof(elem_t));
  else
    init_random_acc(&bias[0], sizeof(bias) / sizeof(elem_t));
  ...
  tiled_conv_auto(BATCH_SIZE,   // batch_size
                  ...
                  (elem_t *)input,                 // input
                  (elem_t *)weights_mat,           // weights
                  NO_BIAS ? NULL : (elem_t *)bias, // bias
                  (elem_t *)output_mat,            // output
