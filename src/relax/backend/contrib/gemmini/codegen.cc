 ...
 * \brief Gemmini JSON codegen implementation based on Example NPU.
 */
...
class GemminiJSONSerializer : public JSONSerializer {
 public:
  GemminiJSONSerializer(ffi::Map<Constant, ffi::String> constant_names,
...
ffi::Array<ffi::Module> GemminiCompiler(ffi::Array<Function> functions,
  ...
  const auto pf = tvm::ffi::Function::GetGlobalRequired("runtime.GemminiJSONRuntimeCreate");

  for (const auto& func : functions) {
    GemminiJSONSerializer serializer(constant_names, AnalyzeVar2Value(func));
